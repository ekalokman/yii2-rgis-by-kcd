...
- Enh: Custom side menu via `ekalokman\AdminPgsql\Module::items` (mdmunir).
...
- Chg: Can customize return of `ekalokman\AdminPgsql\components\AccessHelper::getAssignedMenu()` with provide a callback to method (mdmunir).
...
- Chg: Remove `ekalokman\AdminPgsql\components\BizRule` (mdmunir).
...
- Chg: Remove `ekalokman\AdminPgsql\Module::allowActions`. Set access control directly with `ekalokman\AdminPgsql\components\AccessControl` (mdmunir).
- Chg: Change cache strategy (mdmunir).
- Chg: `ekalokman\AdminPgsql\components\DbManager` now inherited from `yii\rbac\DbManager` (mdmunir).
